...
The design flow of any system consist of design at various levels of
...
The above-mentioned layers can be considered as vertical layers of
any digital system. The BlueBox system could be broken down into
three main subsystem as shown in \FIG{table:sub-system} based on its
...
MA100 can be seen in \FIG{body_temp}.
...
\FIG{AIC_processingBlock}. The codec has a fixed number of filters
...
is under various states of loads using \EQ{eq:idleUtil}. BlueBox
...
I2S bus. From \FIG{main}, it can be seen that the main thread of the
...
\end{cmtPai}
\begin{cmtSMS}
	I wanted to mean that the effect of overhead associated with event reduction could affect the system requirement. For example: Event reduction reduces the number of events, but the could increase the ISR	runtime as more aggregated work needs to be done. This increase in worst case execution time of a particular ISR might affect the schedulability of the other ISR.
\end{cmtSMS}
\end{comment}
...
\FIG{event_reduction} explains how the reduction is achieved.
...
\FIG{body_temp} shows the simplified body temperature measurement
...
Storage contributes to a significant fraction of power consumption of
